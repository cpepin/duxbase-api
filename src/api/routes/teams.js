...
const { createTeam, findTeams } = require('../queries/teams');
...
TeamsRouter.post('/', authenticated, asyncMiddleware(async (req, res) => {
  const schema = {
  ...
  const [savedTeam] = await createTeam({ ...newTeam, ['user_id']: req.user.id });
